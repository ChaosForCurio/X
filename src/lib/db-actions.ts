...
export async function saveMessage(userId: string | null, chatId: string, role: 'user' | 'ai', content: string) {
    try {
        ...
        console.error(`[DB] Error saving message for chat ${chatId}:`, error);
        throw error;
    }
}

export async function getChatHistory(userId: string | null, chatId: string, limit: number = 50) {
    try {
...
export async function isRateLimited(userId: string | null): Promise<boolean> {
    if (!userId) return false; // No rate limit for anonymous for now, or handle by IP? 
    // For now, let's just skip DB rate limit for null userId to avoid errors, 
    // or we could use 'anonymous' string if we want to limit them collectively (bad idea).
    // Let's return false for now to unblock.
...
export async function ensureChat(chatId: string, userId: string | null, title: string = 'New Chat') {
    try {
        await db.insert(chats).values({
            id: chatId,
            userId: userId, // Pass null if null
            title,
        }).onConflictDoNothing();
        // Ensured chat exists
    } catch (error) {
        console.error(`[DB] Error ensuring chat ${chatId}:`, error);
        throw error;
    }
}
